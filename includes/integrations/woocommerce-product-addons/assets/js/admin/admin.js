jQuery( function( $ ) {
	...
	var wc_pao_admin = {
		...
		updateDefaultOptions: function( dropdown, updatedOption, action ) {

			// Get the index of the option, to update the specific option in the dropdown.
			const optionIndex = updatedOption.closest('.wc-pao-addon-option-row').index(),
				  optionValue = updatedOption.val();
			...
			if ( 'change' === action ) {
				// Check if the dropdown already has an option at this index.
				if (dropdown.find(`option:eq(${optionIndex + 1})`).length) { // +1 because the "None" option is at index 0
					// Update the existing option at this index.
					dropdown.find(`option:eq(${optionIndex + 1})`).val(optionValue).text(optionValue);
				} else {
			...
			$( '.wc-pao-addon-content-options-container' ).sortable( {
				...
				stop: function( event, ui ) {
					...
					const $optionsContainer = $( this ),
						  $options          = $optionsContainer.find( '.wc-pao-addon-content-label input' ),
						  $dropdown         = $optionsContainer.closest('.wc-pao-addon-content').find('.wc-pao-addon-default-option-setting select');

					if ( $dropdown.length ) {

						const defaultValue = $dropdown.val();
						$options.each( function() {
							wc_pao_admin.updateDefaultOptions( $dropdown, $(this), 'change' );
						} );

						// Re-select the previously selected option.
						if ( $dropdown.find(`option[value="${defaultValue}"]`).length ) {
							$dropdown.val(defaultValue);
						}
					}

					// Update indexes for default add-on options.
					$optionsContainer.find( 'input.wc-pao-addon-option-default' ).each(function ( index ) {
						var name = $(this).attr('name');

						if (name) {
							// Replace the old index with the new index
							name = name.replace(/\[0\]\[\d+\]/, `[0][${index}]`);
							$(this).attr('name', name);
						}
					});
				}
		...
		validateSettings: function( fields = 'all' ) {
			...
			$( '.wc-pao-addons' ).find( '.wc-pao-addon' ).each( function( i ) {
				...
				if ( fields === 'all' || fields === 'addon_required' ) {
					var $addon = $(this),
						type = $addon.find('.product_addon_type').val();

					if ( type === 'multiple_choice' || type === 'checkbox' ) {
						var $requiredInput = $addon.find('.wc-pao-addon-required-setting input'),
							required = $requiredInput.is(':checked'),
							optionsCount = $addon.find('.wc-pao-addon-option-row').length,
							hiddenOptions = $addon.find('.wc-pao-addon-option-row .wc-pao-addon-content-visibility button.is-hidden').length;

						// If the add-on is required and all options are hidden, then show an error.
						if ( required && optionsCount === hiddenOptions ) {
							$requiredInput.addClass('wc-pao-error');
							$addon.closest('.wc-pao-addon').addClass('wc-pao-error');

							shouldReturn = false;
							removeErrorBorder = false;
							invisibleOptionsError = true;
						} else {
							$requiredInput.removeClass('wc-pao-error');
						}
					}
				}
			...
			if ( false === shouldReturn ) {
				...
				} else if ( invisibleOptionsError ) {
					errorMessage = $( '<div class="notice notice-error wc-pao-error-message"><p>' + wc_pao_params.i18n.no_visible_options + '</p></div>' );
				}
		...
		render: function( selectedValue, $addon ) {
			...
			// Default hide the default column.
			$addon.find( '.wc-pao-addon-content-default' ).removeClass( 'show' ).addClass( 'hide' );
			...
			switch ( selectedValue ) {
				case 'multiple_choice':
					...
					$addon.find( '.wc-pao-addon-main-settings-2 .wc-pao-col2' ).removeClass( 'hide' ).addClass( 'show' );
				...
				case 'checkbox':
					...
					$addon.find( '.wc-pao-addon-content-default' ).removeClass( 'hide' ).addClass( 'show' );
					$addon.find( '.wc-pao-addon-content-headers' ).addClass( 'checkboxes' );
					$addon.find( '.wc-pao-addon-option-row' ).addClass( 'checkboxes' );
					break;
				case 'custom_price':
		...
		init: function() {
			...
			$products_addons_data
				...
				.on( 'click', 'button.wc-pao-add-option', function() {
					...
					var $addon = $( this ).closest( '.wc-pao-addon' ),
						loop   = $addon.find('.wc-pao-addon-content-name').attr('name').replace( /product_addon_name\[([0-9]*)\]/i, '$1' ),
						parent = $( this ).parents( '.wc-pao-addon-content-option-rows' ).find( '.wc-pao-addon-content-options-container' ),
						index = parent.find( '.wc-pao-addon-option-row' ).length;
						...
						$.when( wc_pao_admin.getAddonOptions() ).then( function( html ) {
							...
							if ( 'checkbox' === $addon.find( '.product_addon_type' ).val() ) {
								$addon.find( '.wc-pao-addon-content-default' ).removeClass( 'hide' ).addClass( 'show' );
								$addon.find( '.wc-pao-addon-content-headers' ).addClass( 'checkboxes' );
								$addon.find( '.wc-pao-addon-option-row' ).addClass( 'checkboxes' );
							}
				...
				.on( 'click', '.wc-pao-remove-option', function() {
					...
					if ( answer ) {
						var $addon = $option.parents( '.wc-pao-addon' ),
							$dropdown = $option.closest('.wc-pao-addon-content').find('.wc-pao-addon-default-option-setting select'),
							removeAddOnOptionButtons;

						if ( $dropdown.length ) {
							// Update the default options dropdown, when an option is removed.
							wc_pao_admin.updateDefaultOptions( $dropdown, $option.closest('.wc-pao-addon-option-row').find('.wc-pao-addon-content-label input'), 'remove' );
						}

						$option.parents( '.wc-pao-addon-option-row' ).remove();
						// Count the number of options.  If one (or less), disable the remove option buttons
				...
				.on( 'click', '.wc-pao-addon-content-visibility', function() {
					var $option = $(this),
					...
					// Validate that not all options have been hidden for required add-ons.
					wc_pao_admin.validateSettings( 'addon_required' );

					// Update option tooltip.
					var newContent = $button.hasClass( 'is-hidden' ) ? wc_pao_params.i18n.options_show_option : wc_pao_params.i18n.options_hide_option;
					$option.attr( 'title', newContent );

					$option.tipTip( 'hide' ).tipTip( 'show' );
				} )
				.on( 'click', '.wc-pao-addon-required-setting', function() {
					// Validate that not all options have been hidden for required add-ons.
					wc_pao_admin.validateSettings( 'addon_required' );
				} )
				...
				.on( 'click', '.wc-pao-addon-placeholder-enable', function() {
					if ( $( this ).is( ':checked' ) ) {
						$( this ).parents( '.wc-pao-addons-secondary-settings' ).find( '.wc-pao-addon-placeholder' ).removeClass( 'hide' ).addClass( 'show' );
					} else {
						$( this ).parents( '.wc-pao-addons-secondary-settings' ).find( '.wc-pao-addon-placeholder' ).removeClass( 'show' ).addClass( 'hide' );
					}
				} )
				...
				.on( 'change', '.wc-pao-addon-content-options-container .wc-pao-addon-content-label input', function(e) {
					const $dropdown = $(this).closest('.wc-pao-addon-content').find('.wc-pao-addon-default-option-setting select');
			...
			$products_addons_data.on( 'click', '.wc-pao-import-addons', function( e ) {
				...
				// Global-level add-ons.
					...
					// The edit URL parameter is present only for existing add-ons.
					post_id = url_params.get( 'edit' );

					if ( ! post_id ) {
						post_id = url_params.get( 'add' );
					}
			...
			// Initialize tooltips on all add-on option visibility settings.
			$('.wc-pao-addon-content-visibility button').each(function() {
				var $self = $(this);
				...
				$self.tipTip({
					...
					delay: 50,
					content: function() {
						return $self.hasClass('is-hidden') ? wc_pao_params.i18n.options_show_option : wc_pao_params.i18n.options_hide_option;
					}
				});
			});
